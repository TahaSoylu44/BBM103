...
def bubble_sort(dataset):
    ...
    j=0
    while j<number_of_elements:                 #Swapping time!
...
def merge_sort(mylist):
    ...
    left_subset=merge_sort(subset1)      #Recursively dividing
...
def quick_sort(mylist):
    ...
    for i,k in zip(mylist,mylist[1:]):    #While comparison,I have 2D comparison situation.If correctness remains true,it means I will compare 3.index
        if i[1]!=k[1]:
            correctness=False
            break

    if correctness:         #Comparing 3.index
        counter+=1
