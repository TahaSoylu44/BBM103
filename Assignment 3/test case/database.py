...
def main():
    with open(argv[1].strip(),"r",encoding="utf-8") as file:
        lines=file.readlines()           #Read the document line by line
        command_list=[]                  #It keeps the table names
        joining=[]                        #A sublist for join operations
        joined_list=[]                    #It keeps the necessary datas for join operations
    for line in lines:
        if line!="\n":
    ...
    def data_func(name):              #This function returns a list which includes both the name of the table and datas as dictionaries.
        ...
        conditions=[]        #A sublist for selecting operations
        condition_list=[]    #It keeps the datas for selecting operations
        updating=[]          #It keeps the datas for updating operations
        deleting=[]          #It keeps the datas for deleting operations
        counting=[]          #It keeps the datas for counting operations
        with open(argv[1].strip(),"r",encoding="utf-8") as file:
                ...
                if line!="\n":
                    ...
                    if splitted_line[1]==name:
                        if splitted_line[0]=="DELETE":
                            ...
                            counter=0
                            for index,character in enumerate(line):        #enumerating faciliated my work to keep the required datas from the lines.
                                if character==" ":
                                    counter+=1
                                    if counter==3:
...
def updated(table):   #It will update my datalist according to the datas of input files.
...
def INSERT(table):    #It is for inserting.
...
def CREATE(table):        #Constituting the tables.
    print(22*"#"+" "+"CREATE"+" "+25*"#")
    print(f"Table '{table[0]}' created with columns: {list(table[1].keys())}")
    print(55*"#")
    print()

def SELECT(table):        #Selecting the opeartions according to the conditions such as "WHERE {something:"something"}"
    selected=[]
    SubList=[]
...
def UPDATE(table):        #In "update(table)" part,I built my table but I need to print it with some sentences.
        ...
        try:
            ...
            for dictionary in mydict:
                for key in dictionary.keys():
                    ...
                    widths.append(width)
                break
            def bars():              #It will arrange bars such as "+--------"
                nonlocal columns
                nonlocal last_item
                k=0
                while k<4:
...
def DELETE(table):          #For deleting operations
    nonexist_key=None
    if main()[0](name)[2]!=[]:
...
def COUNT(table):             #Counting according to the conditions.
    no_column=None
    if main()[0](name)[2]!=[]:
        DataList=updated(table)[0]
        for i in main()[0](name)[3][-1].keys():
...
def JOIN():       #Join operations for two tables an a common column.
    if len(main()[2])>=1:
...
if __name__=="__main__":
    for tuples in main()[1]:             #The lasting I have to do is to print the output by using my functions :D
        name=tuples[0]
        main()[0](name)
